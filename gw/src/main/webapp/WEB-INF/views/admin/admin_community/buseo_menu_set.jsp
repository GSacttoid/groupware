				...
				<div class="main-wrap noto font14" style="width: 100%">
					...
					<div class="p20">
						<div class="notice-title font16 noto">
							<h3>
								<i class="fas fa-info-circle m-lr5"></i>부서 게시판 관리
							</h3>
						</div>
						<hr />
							...
							<tr>
								<td style="width: 20%; line-height: 1.8; padding: 10px;">
									<div class="noto font14">
										<ul>
											<li>부서게시판의 모든 게시판은 해당 부서 직원에게 공개됩니다.</li>
						...
						<div class="flex flex-justify">
			            	<div style="width:100%; margin-right:10px;">
			           	        <form method="post" id="buseoList" >
						            <div class="p10" style="border: 1px solid #c1c1c1;">
						                <div class="">
						                    <div class="noto font16 weight700 m-tb10">1. 부서 게시판 리스트</div>
						                	...
						                	<div class="">
						                    	<table style="width:100%; border: 1px solid #c1c1c1; margin-bottom: 10px;">
						                              <tr class="center noto font14 bg-gray" id="rowColor" style="height: 40px; border: 1px solid #c1c1c1;">
														  ...
														  <td id="fontColor" style="border: 1px solid #c1c1c1; width:15%">부서명</td>
						                              ...
						                              <c:forEach var="buseoMenuList" items="${buseoMenuList}" varStatus="status">
							                              <tr class="center noto font14">
							                                  <td>
							                                      <input type="checkbox" name="chk" class="chk" data-uid="${buseoMenuList.menu_code}" style="width:17px; height:17px;" />
							                                  </td>
							                                  <td>${buseoMenuList.menu_buseo}</td>
							                                  <td>${buseoMenuList.menu_kind}</td>
							                                  <td>${buseoMenuList.menu_name}</td>
							                                  <td>${buseoMenuList.menu_regdate}</td>
							                                  <td>
							                                  	<button class="s-btn-off" type="button" onClick="boardDelete('${buseoMenuList.menu_code}');" 
							                    	...
							                    	<div class="page-grp center m-t10">
							                         	<c:choose>
							                               	<c:when test = "${curPage > 1}">
																<span class="page">
							                                     	<a href="${pageContext.request.contextPath}/admin/buseo_menu_set?curPage=1"><i class="fas fa-angle-double-left"></i></a>
							                                 	</span>
							                             	</c:when>
							                             	<c:otherwise>
							                                	<span class="page">
							                             ...
							                             <c:choose>
							                               	<c:when test = "${curPage > 1}">
							                               		<span class="page">
							                                    	<a href="${pageContext.request.contextPath}/admin/buseo_menu_set?curPage=${curPage-1}"><i class="fas fa-angle-left"></i></a>
							                                	</span>
							                               	</c:when>
							                             	<c:otherwise>
								                               	<span class="page">
							                           	...
							                           	<c:forEach begin="${blockBegin}" end ="${blockEnd}" var="num">
							                           		<c:if test="${selected != num}">
							                           			<a href="${pageContext.request.contextPath}/admin/buseo_menu_set?curPage=${num}">
							                           ...
							                           <c:choose>
							                               	<c:when test = "${curPage != totalPage }">
							                               		<a href="${pageContext.request.contextPath}/admin/buseo_menu_set?curPage=${curPage+1}">
							                               			<span class="page">
							                     		...
							                     		<c:choose>
							                               	<c:when test = "${curPage != totalPage }">
							                               		<span class="page">
							                                   		<a href="${pageContext.request.contextPath}/admin/buseo_menu_set?curPage=${totalPage}"><i class="fas fa-angle-double-right"></i></a>
							                               		</span>
							                             	</c:when>
							                             	<c:otherwise>
							                               		<span class="page">
			            	...
			            	<div style="width:100%">
			           	        <form method="post" id="frm" autocomplete="off">
						            <div class="p10" style="border: 1px solid #c1c1c1;">
						            	<div class="noto font16 weight700 m-tb10">2. 부서 게시판 만들기</div>
						                <table style="width:100%; border: 1px solid #c1c1c1; margin-bottom: 10px;" >
						                	<tr class="m-t10">
						                        <td class="font14" style="width: 20%; text-align: center; background-color: #e2e2e2; padding: 10px;">게시판 구분</td>
						                        <td>
						                        	<select class="sel-135" id="menu_buseo" name="menu_buseo" style="margin-left: 3px;">
...
<script>
	function boardMake() {
		...
		$.ajax({
			url		: "${pageContext.request.contextPath}/board/buseo_menu_insert",
			type	: "POST",
			data	: $("#frm").serialize(),
			success	: function(resData) {
				alert( $("#menu_name").val() + "게시판 & 댓글게시판이 생성되었습니다.");
				window.location.href = 	"${pageContext.request.contextPath}/admin/buseo_menu_set";			
			},
			error	:function() {
				alert("시스템 에러");			
			}
...
function menuDeleteAll() {
    ...
    if( str ) {
        ...
        $.ajax({
        	url 	: "${pageContext.request.contextPath}/board/buseo_menu_delete_all",
        	type 	: "POST", 	
        	data 	: { chkArr : chkArr },
        	success	: function (resData) {
        		alert("삭제되었습니다.");
...
function boardDelete(menu_code) {
    ...
    if( str ) {      
        $.ajax({
        	url 	: "${pageContext.request.contextPath}/board/buseo_board_delete",
        	type 	: "POST", 	
...
<script>
	function chkCode() {
		...
		$.ajax({
        	url 	: "${pageContext.request.contextPath}/board/buseo_menu_check",
        	type 	: "POST", 	
        	data 	: formData,
        	success	: function (resData) {
            	if( resData == "failure" ) {
