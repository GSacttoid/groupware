							...
							<div class="page-wrap bg-white" style="padding: 20px 0;">
								<div class="title">
									<h3 class="font18 noto m-lr10" style="color:tomato;">
										<i class="fas fa-feather-alt"></i><span> 회사 공지사항 수정하기</span>
									</h3>
								</div>
								<div class="board-insert p10">
									<form id="frm" name="frm" method="post" enctype="multipart/form-data" onsubmit="return false;" autocomplete="off">
										...
										<div class="btn-grp center m-t10">
											<button type="submit" id="btn" name="btn" class="btn-on" onClick="noticeModify(${notice.sn_num});">수정</button>
											<button type="button" onClick="location.href='${pageContext.request.contextPath}/admin/company_notice'" class="btn-red">목록보기</button>
										</div>
									</form>
								</div>
							</div>
...
function noticeModify(sn_num){
	var str = confirm("정보를 수정하시겠습니까?");
    if( str ) { 
		CKEDITOR.instances.editor.updateElement();
		var formData = $("#frm").serialize();
		$.ajax({
			url		: "${pageContext.request.contextPath}/admin/company_notice_modify",
			type	: "POST",
			data	: formData,
